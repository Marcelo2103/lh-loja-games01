...
export class AtualizaProdutoComponent implements OnInit {
  ...
  listarProduto(): void {
    this._produtoService.getProduto(this.produtoId).subscribe((res:any) => {console.log(res[0].produto);
    this.produto = new
  ...
  atualizar(id: number) {
    this._produtoService.atualizarProduto(id,this.produto).subscribe((_produto: any) => {this.produto = new Produto(0,"","","",0)},
